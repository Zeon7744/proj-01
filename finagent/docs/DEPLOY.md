...
```bash
cp .env.example .env
# 编辑 .env，填入 ALPHAVANTAGE_API_KEY
export ALPHAVANTAGE_API_KEY=xxxx   # 或在 .env 中配置
```
...
每个接入应用持有一个 API Key + 作用域（scope），用于权限隔离：

```bash
# 为一个“应用”签发密钥（admin）
curl -X POST http://localhost:3001/api/keys/analyst \
...
- 设置 `FINAGENT_ADMIN_KEY` 为随机值，弃用默认 `fa_admin`。
- 设置 `FINAGENT_LOCAL_OPEN=0`，关闭“无 key 降级 admin”。
- 将 `data/` 指向对象存储或数据库；密钥改放 KMS / 环境变量而非代码。
- 鉴权已内置；如需多租户，扩展 `src/auth/rbac.js` 的 ACCOUNTS 与中间件即可。

## 5. 定时迭代（可选）

采集/分析是幂等的增量任务，可用 cron / GitHub Actions schedule 周期运行：
...
每次运行都会：多源采集 → 自研高精度模型 → 命中回测（含历史 dry-run）→ 写回 adaptive 反馈 → 自我升级。
